"""Shared network building blocks for the brax-style agents."""
...
def make_inference_fn(agent_networks):
    def make_policy(params: types.PolicyParams, deterministic: bool = False) -> types.Policy:
        def policy(observations: types.Observation, key_sample: PRNGKey) -> Tuple[types.Action, types.Extra]:
            logits = agent_networks.policy_network.apply(*params, observations)
            if deterministic:
                return agent_networks.parametric_action_distribution.mode(logits), {}
            return (
                agent_networks.parametric_action_distribution.sample(logits, key_sample),
                {},
            )
