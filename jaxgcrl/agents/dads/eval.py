...
def make_dads_eval_policy(
    ...
    so that the trained DADS policy (which expects ``[state | z]``) can be used
...
def run_multi_skill_evaluation(
    ...
    Returns aggregated metrics with per-skill success rates prefixed by skill
    index.  Follows the same pattern as DIAYN evaluation.
    ...
    Args:
        ...
        training_metrics:  Training metrics to pass through to the Evaluator.

    Returns:
        Dictionary of aggregated eval metrics with per-skill success rates.
    """
    all_metrics = {}

    # Collect metric values per base key for aggregation across skills.
    ...
    for skill_idx in range(num_skills):
        ...
        # Per-skill success rates under a dedicated prefix.
        ...
        # Collect non-std, non-success metrics for cross-skill aggregation.
        for key, value in skill_metrics.items():
            if (
    ...
    # Aggregate: mean only, no std, no per-skill breakdown for non-success metrics.
