...
# seeds, at controller_target_entropy_scale=0.25. This is the multi-seed
...
#
# THIS IS THE 0.25 HALF of a two-script entropy sweep, on NORMAL priority. The
# other half — controller_target_entropy_scale=0.5 — is
# rail_slurm_skill_commitment_crl_dads_dds_seeds_ent05_low.sh on LOW priority.
...
# controller's skill distribution, so the 0.5 arm holds the controller at a
# more uniform (more exploratory) skill mixture than this one. Being a fraction
...
# HALF OF THE SWEEP — see the ent05_low script for the other value.
ENT=0.25
